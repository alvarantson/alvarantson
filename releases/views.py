	...
	if request.user.is_authenticated:
		...
		if request.POST:
			...
			if request.POST["submit-btn"] == "search":
				...
				if artist != "":
					list_of_releases_unsorted = list_of_releases_unsorted.filter(artist__contains=artist)

				if name != "":
					list_of_releases_unsorted = list_of_releases_unsorted.filter(name__contains=name)

				if role != "":
					list_of_releases_unsorted = list_of_releases_unsorted.filter(role__contains=role)

				if genre != "":
					list_of_releases_unsorted = list_of_releases_unsorted.filter(genre__contains=genre)
		...
		return render(request, "releases_public.html", {"releases":releases})
...
def tests(request):
	reliis = Release.objects.filter(artist__contains="EmM").first()
	print(reliis)
